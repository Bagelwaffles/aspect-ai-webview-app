...
import {
  buildNurtureContentBrief,
  type NurtureLength,
  type NurtureTone,
  type NurtureType,
} from "@/lib/nurture-workflow"
...
function createRequestKey(): string {
  return typeof crypto !== "undefined" && "randomUUID" in crypto
    ? `nurture-agent-${crypto.randomUUID()}`
    : `nurture-agent-${Date.now()}-${Math.random().toString(36).slice(2, 14)}`
}
...
export default function NurtureAgentPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const brief = buildNurtureContentBrief({
      ...
      campaignType: String(form.get("campaignType") ?? "welcome") as NurtureType,
      sequenceLength: String(form.get("sequenceLength") ?? "3") as NurtureLength,
      objective: String(form.get("objective") ?? ""),
      keyMessage: String(form.get("keyMessage") ?? ""),
      tone: String(form.get("tone") ?? "professional") as NurtureTone,
      ...
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-6">
        <header className="flex flex-col gap-4 border-b pb-5 sm:flex-row sm:items-start sm:justify-between">
          <div className="space-y-2">
            <Badge variant="secondary" className="w-fit">Beta · draft workflow · production verification pending</Badge>
            <div>
          ...
          <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row">
            <Button asChild variant="outline" className="h-11 w-full sm:w-auto">
              <Link href="/agents/nurture-agent"><ArrowLeft className="mr-2 h-4 w-4" />Agent status</Link>
            </Button>
            <Button asChild variant="outline" className="h-11 w-full sm:w-auto">
        ...
        <section className="grid items-start gap-6 lg:grid-cols-[minmax(0,1.05fr)_minmax(20rem,0.95fr)]">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Draft generation only; no lifecycle events or delivery service are connected. One successful generation uses one Content Agent credit.</CardDescription>
            </CardHeader>
            <CardContent>
              <form
                className="space-y-5"
              ...
              >
                <div className="space-y-2">
                  <Label htmlFor="nurture-business">Business name</Label>
                  <Input id="nurture-business" className="h-11 text-base" name="businessName" autoComplete="organization" inputMode="text" maxLength={120} minLength={2} required />
                </div>

                <div className="space-y-2">
                  <Label htmlFor="nurture-audience">Audience</Label>
                  <Textarea id="nurture-audience" className="min-h-24 text-base" name="audience" placeholder="Example: Kentucky small-business owners who opted in for marketing tips" autoCapitalize="sentences" spellCheck maxLength={500} minLength={3} required />
                </div>

                <div className="grid gap-4 sm:grid-cols-2">
                  <div className="space-y-2">
                    <Label htmlFor="nurture-type">Lifecycle stage</Label>
                    <select id="nurture-type" name="campaignType" defaultValue="welcome" className="flex h-11 w-full rounded-md border border-input bg-background px-3 py-2 text-base">
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="nurture-length">Sequence length</Label>
                    <select id="nurture-length" name="sequenceLength" defaultValue="3" className="flex h-11 w-full rounded-md border border-input bg-background px-3 py-2 text-base">
                ...
                <div className="space-y-2">
                  <Label htmlFor="nurture-objective">Lifecycle objective</Label>
                  <Textarea id="nurture-objective" className="min-h-20 text-base" name="objective" placeholder="Example: Introduce the $49 Quick Marketing Audit and earn qualified replies" autoCapitalize="sentences" spellCheck maxLength={240} minLength={3} required />
                </div>

                <div className="space-y-2">
                  <Label htmlFor="nurture-message">Key message</Label>
                  <Textarea id="nurture-message" className="min-h-24 text-base" name="keyMessage" placeholder="What should readers understand by the end of the sequence?" autoCapitalize="sentences" spellCheck maxLength={300} minLength={3} required />
                </div>

                <div className="grid gap-4 sm:grid-cols-2">
                  <div className="space-y-2">
                    <Label htmlFor="nurture-tone">Tone</Label>
                    <select id="nurture-tone" name="tone" defaultValue="professional" className="flex h-11 w-full rounded-md border border-input bg-background px-3 py-2 text-base">
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="nurture-offer">Offer (optional)</Label>
                    <Input id="nurture-offer" className="h-11 text-base" name="offer" placeholder="$49 Quick Marketing Audit" inputMode="text" maxLength={500} />
                  </div>
                </div>

                <div className="space-y-2">
                  <Label htmlFor="nurture-constraints">Constraints or must-include details (optional)</Label>
                  <Textarea id="nurture-constraints" className="min-h-20 text-base" name="constraints" placeholder="Example: No discounts; keep each email concise." autoCapitalize="sentences" spellCheck maxLength={200} />
                </div>
                ...
                <Button className="h-11 w-full sm:w-auto" disabled={submitting} type="submit">
                  ...
                  {submitting ? "Building nurture sequence" : "Generate nurture sequence"}
                </Button>
              </form>
            </CardContent>
          </Card>
