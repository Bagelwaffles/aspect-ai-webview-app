...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${BACKEND_URL.replace(/\/$/, "")}/internal/stripe/checkout-session`, {
      method: "POST",
      headers: {
    ...
    return NextResponse.json(json ?? { error: "Checkout failed" }, { status: response.status });
  } catch (error) {
